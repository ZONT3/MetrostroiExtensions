                    ...
                    for i, stbl in ipairs(v) do
                        ...
                        if snd:GetState() ~= GMOD_CHANNEL_PLAYING and stbl.volume ~= 0 and not stbl.state then
                            ...
                            if i == 1 then
                                ...
                                local endt
                                if k ~= "horn" then endt = (ntbl.loop and snd:GetTime() > ntbl.loop or snd:GetTime() / snd:GetLength() >= 0.8) or no1 end
                            ...
                            if i == 3 then
                                ...
                                local hlt = wagon._HornLoopTime3 or CurTime()
                                ...
                                if stbl.state and time and not stbl.time then
                                    if snd:GetState() ~= GMOD_CHANNEL_PLAYING then
                                        snd:Play()
                                        wagon:SetBASSPos(snd, tbl)
                                        if k == "horn" then wagon._HornLoopTime3 = CurTime() end
                                    end
                                ...
                                elseif (not stbl.state or timelim) and stbl.time then
                                    stbl.time = nil
                                    stbl.state = false
                                    if k == "horn" then wagon._HornLoopTime3 = false end
                                end
                            end
                        end
                    end
            ...
            elseif coeff > 1.3 then
                ...
                if wagon.RenderClientEnts and wagon.PassengerEnts then
                    ...
                    for i, v in ipairs(wagon.LeftDoorPositions) do
                        ...
                        elseif IsValid(stucked[i]) and (not wagon:GetPackedBool("DoorLS" .. i) or stucked[i]:IsDormant()) then
                    ...
                    for i, v in ipairs(wagon.RightDoorPositions) do
                        ...
                        elseif IsValid(stucked[-i]) and (not wagon:GetPackedBool("DoorRS" .. i) or stucked[-i]:IsDormant()) then
                            SafeRemoveEntity(stucked[-i])
                        end
                    end
                    ...
                    if #wagon.PassengerEnts ~= wagon:GetNW2Float("PassengerCount") then
                        ...
                        while #wagon.PassengerEnts > wagon:GetNW2Float("PassengerCount") do
                            local tbl = wagon.PassengerEnts[#wagon.PassengerEnts]
                        ...
                        while #wagon.PassengerEnts < wagon:GetNW2Float("PassengerCount") do
                            ...
                            local tbl = {}
                            tbl.mdl = table.Random(wagon.PassengerModels)
                            tbl.pos = pos
                        ...
                        hook.Add("MetrostroiBigLag",ent,function(ent)
                            ...
                            table.insert(wagon.PassengerEnts, tbl)
                        end
                    end

                    if CurTime() - (wagon.ModelCheckTimer or 0) > 1.0 then
                        wagon.ModelCheckTimer = CurTime()
                        local plyPos = LocalPlayer():GetPos()
                        local dist = C_PassRenderDistance:GetInt() / 0.01905
                        for _, tbl in pairs(wagon.PassengerEnts) do
                            if not IsValid(tbl.ent) and plyPos:Distance(tbl.pos) <= dist then
                                local ent = ClientsideModel(tbl.mdl, RENDERGROUP_OPAQUE)
                                if not IsValid(ent) then continue end
                                ent:SetPos(wagon:LocalToWorld(tbl.pos))
                                ent:SetAngles(wagon:LocalToWorldAngles(tbl.ang))
