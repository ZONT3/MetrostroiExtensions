...
--------------------------------------------------------------------------------
Metrostroi.DefineSystem("81_717_Pneumatic_EXT")
...
function TRAIN_SYSTEM:Initialize(parameters)
    ...
    self.Train:LoadSystem("UAVAC", "Relay", "", {
...
function TRAIN_SYSTEM:Outputs()
    return {"BrakeLinePressure", "BrakeCylinderPressure", "DriverValvePosition", "ReservoirPressure", "TrainLinePressure", "DoorLinePressure", "WeightLoadRatio", "WorkingChamberPressure", "AuxiliaryLinePressure"}
end
...
function TRAIN_SYSTEM:Think(dT)
    ...
    local pr_speed = 1 --[[BLDisconnect,]] --[[true,]]
    ...
    if self.ValveType == 1 then
        ...
        if self.RealDriverValvePosition > 2 and self.RealDriverValvePosition < 5 then
            ...
            for _i = 1, #Train.WagonList do
                if Train.WagonList[_i].Pneumatic.TLDisconnect and Train.WagonList[_i].Pneumatic.BLDisconnect and (Train.WagonList[_i].Pneumatic.RealDriverValvePosition == 2 or Train.WagonList[_i].Pneumatic.RealDriverValvePosition == 1) then _a = _a + 1 end
        ...
        if not self.TLDisconnect then self.TLDisconnectPressure = math.max(0, self.TLDisconnectPressure - math.abs(self.ReservoirPressure_dPdT) * dT) end
        ...
        -- 013: 1 Overcharge
        if self.RealDriverValvePosition == 1 and self.BLDisconnect and (self.TLDisconnect or self.BrakeLinePressure > self.TrainLinePressure) then self:equalizePressure(dT, "BrakeLinePressure", math.min(6.4, self.TrainLinePressure), pr_speed, Train.EPKC and Train.EPKC.Value == 0 and Train.EPK.Value > 0 and pr_speed * 2.2 or pr_speed * 0.35, nil, 1.0) end
        ...
        -- 013: 3 4.3 Atm
        if self.RealDriverValvePosition == 3 and self.BLDisconnect and (self.TLDisconnect or self.BrakeLinePressure > 1.01 * math.min(4.3, self.TrainToBrakeReducedPressure)) then self:equalizePressure(dT, "BrakeLinePressure", 1.01 * math.min(4.3, self.TrainToBrakeReducedPressure), pr_speed, pr_speed * 0.35, nil, 1.0) end
        -- 013: 4 4.0 Atm
        if self.RealDriverValvePosition == 4 and self.BLDisconnect and (self.TLDisconnect or self.BrakeLinePressure > 1.01 * math.min(4.0, self.TrainToBrakeReducedPressure)) then self:equalizePressure(dT, "BrakeLinePressure", 1.01 * math.min(4.0, self.TrainToBrakeReducedPressure), pr_speed, pr_speed * 0.35, nil, 1.0) end
        -- 013: 5 3.7 Atm
        if self.RealDriverValvePosition == 5 and self.BLDisconnect and (self.TLDisconnect or self.BrakeLinePressure > 1.01 * math.min(3.7, self.TrainToBrakeReducedPressure)) then self:equalizePressure(dT, "BrakeLinePressure", 1.01 * math.min(3.7, self.TrainToBrakeReducedPressure), pr_speed, pr_speed * 0.35, nil, 1.0) end
        -- 013: 6 3.0 Atm
        if self.RealDriverValvePosition == 6 and self.BLDisconnect and (self.TLDisconnect or self.BrakeLinePressure > 1.01 * math.min(3.0, self.TrainToBrakeReducedPressure)) then self:equalizePressure(dT, "BrakeLinePressure", 1.01 * math.min(3.0, self.TrainToBrakeReducedPressure), pr_speed, pr_speed * 0.35, nil, 1.0) end
        -- 013: 7 0.0 Atm
        if self.RealDriverValvePosition == 7 and self.BLDisconnect and (self.TLDisconnect or self.BrakeLinePressure > 0.0) then self:equalizePressure(dT, "BrakeLinePressure", 0.0, 0.6 + pr_speed * math.exp(math.min(0, self.BrakeLinePressure - 2.8) * 1.0), pr_speed * 0.35, nil, 1.0) end
        trainLineConsumption_dPdT = trainLineConsumption_dPdT + math.max(0, self.BrakeLinePressure_dPdT)
    end
    ...
    if HaveEPK and Train.EPKC then
        ...
        if self.BLDisconnect and Train.EPK.Value > 0 then
            if Train.EPKC.Value > 0 then
                self:equalizePressure(dT, "EPKPressure", self.BrakeLinePressure, math.min(1, epkDiff) * 6, --[[ pr_speed*math.min(1,epkDiff)*2--]]
                    math.min(1, epkDiff) * 26, false, 4 * epkDiff * 2)
            end
        ...
        if Train.EPK.Value == 0 or Train.EPKC.Value == 0 then leak = leak + self:equalizePressure(dT, "EPKPressure", 0, 16, false, false, 5) end
        if self.ValveType == 2 and not self.BLDisconnect then self:equalizePressure(dT, "EPKPressure", 0, 16, false, false, 5) end
    ...
    if (Train.UAVA.Blocked > 0) ~= UAVABlocked then Train.UAVA:TriggerInput("Block", UAVABlocked and 1 or 0) end
    local UAVACBlocked = self.EmergencyValve and not self.EmergencyValveDisable
    if (Train.UAVAC.Blocked > 0) ~= UAVACBlocked then Train.UAVAC:TriggerInput("Block", UAVACBlocked and 1 or 0) end
    ...
    if Train.AirDistributorDisconnect.Value == 0 and aird_ready then
        ...
        if Train.PneumaticNo1.Value == 1.0 and Train.PneumaticNo2.Value == 0.0 then
            if self.PN1 < self.GN1Offset then self.PN1 = math.min(self.TrainLinePressure, self.GN1Offset) end
        ...
        if Train.PneumaticNo2.Value == 1.0 then
            ...
            if self.BePN2 == false and self.BrakeCylinderPressure > 1.6 then Train:PlayOnce("PN2end", "stop") end
            self.BePN2 = true
        ...
        if self.ValveType == 2 or Train.SubwayTrain.Manufacturer == "LVZ" and not self.BLDisconnect then --and not Train.AR63 then
            ...
            WcBl = self.BrakeLinePressure < 3.2 and 0.45 * (self.WorkingChamberPressure - 2.2) or self.BrakeLinePressure > br013_upper_threshold and self.BrakeLinePressure * (self.BrakeLinePressure > 4.5 and self.BrakeLinePressure_dPdT > 0.02 and 1.06 or 1) --[[3.55]]
        else
        ...
        if math.abs(self.BrakeCylinderPressure - targetPressure) < 0.025 then self.BrakeCylinderValve = 0 end
        if self.BrakeCylinderValve == 1 then self:equalizePressure(dT, "BrakeCylinderPressure", math.min(self.GN2Offset + self.WeightLoadRatio * 1.3, targetPressure), 1, 3.5, nil, 1) end
    ...
    if self.BrakeCylinderPressure < 1 and self.BrakeCylinderPressure_dPdT < -0.1 and self.BrakeEngaged then self.BrakeEngaged = false end
    ...
    if self.DoorLinePressure > 3.5 then
        if Train.VDOL.Value == 1.0 and Train.VDOP.Value == 0.0 and not self.DoorLeft then self.DoorLeft = true end
        if Train.VDOL.Value == 0.0 and Train.VDOP.Value == 1.0 and not self.DoorRight then self.DoorRight = true end
        if (Train.VDZ.Value == 1.0 or Train.VDOL.Value == 1.0 and Train.VDOP.Value == 1.0 or self.RZDTimer) and (self.DoorLeft or self.DoorRight) then
            if not self.OpenWaitL or CurTime() - self.OpenWaitL < 0.2 then self.DoorLeft = false end
            if not self.OpenWaitR or CurTime() - self.OpenWaitR < 0.2 then self.DoorRight = false end
        else
            self.CloseValue = nil
        end
    ...
    for i = 1, 4 do
        ...
        if not Train.LeftDoorsOpen and self.LeftDoorState[i] > 0 then Train.LeftDoorsOpen = true end
        ...
        if not Train.RightDoorsOpen and self.RightDoorState[i] > 0 then Train.RightDoorsOpen = true end
        Train:SetPackedRatio("DoorL" .. i, self.LeftDoorState[i])
        Train:SetPackedRatio("DoorR" .. i, self.RightDoorState[i])
        if self.LeftDoorStuck[i] and (self.DoorLeft or type(self.LeftDoorStuck[i]) == "number" and CurTime() - self.LeftDoorStuck[i] > 0) then self.LeftDoorStuck[i] = false end
        if self.RightDoorStuck[i] and (self.DoorRight or type(self.RightDoorStuck[i]) == "number" and CurTime() - self.RightDoorStuck[i] > 0) then self.RightDoorStuck[i] = false end
    ...
    if CurTime() - self.Timer > 0.10 and self.DriverValvePosition > self.RealDriverValvePosition then
        ...
        if not ValveType then
            if self.RealDriverValvePosition ~= 3 then Train:PlayOnce("br_334", self.RealDriverValvePosition .. "-" .. self.RealDriverValvePosition + 1) end
        else
    ...
    if CurTime() - self.Timer > 0.10 and self.DriverValvePosition < self.RealDriverValvePosition then
        self.Timer = CurTime()
        if not ValveType then
            if self.RealDriverValvePosition ~= 5 then Train:PlayOnce("br_334", self.RealDriverValvePosition .. "-" .. self.RealDriverValvePosition - 1) end
        else
            Train:PlayOnce("br_013", "cabin")
        end
