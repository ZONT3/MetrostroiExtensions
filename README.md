...
- [API](#api)
  ...
  - [Глобальные переменные, определенные рецептами.](#глобальные-переменные-определенные-рецептами)
  - [Методы](#методы)
...
Специально для владельцев серверов, которые по определенной причине не хотят использовать какой-либо из рецептов Metrostroi Extensions и прочих аддонов, использующих библиотеку, существует возможность выборочно отключить каждый рецепт. Для этого следует задать значение 0 ConVar'у данного рецепта. Все ConVar можно посмотреть написав в клиентскую консоль `metrostroi_ext_`. После префикса сначала идет тип энтити для инжекта, а потом название рецепта. В helptext ConVar (можно просмотреть, если не указывать значение, а просто написать имя ConVar) указано описание рецепта (Если оно задано автором).
...
## Глобальные переменные, определенные рецептами.
Некоторые рецепты определяют свои глобальные переменные. Изменять их нужно в `RECIPE:BeforeInject(entclass)`. Вот описание таких переменных, добавленных разными стандартными рецептами:
* KVList - типы кв. Таблица таблиц. Формат - {имя_поля_в_спавнере, путь_до_модели}
* VoltmetersList - типы вольтметров. Таблица таблиц. Формат:
...
`MEL.AddSpawnerField(ent_or_entclass, field_data, is_list_random)` - добавляет новое поле в спавнер. Автоматически передобавляет его при перезагрузке.
