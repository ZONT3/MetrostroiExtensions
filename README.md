<h1 align="center">
MetrostroiExtensions - простой способ написать инжект!
</h1>
MetrostroiExtensions добавляет библиотеку для инжекта в поезда аддона Metrostroi.
Многие вещи, о которых забывают создатели инжектов MetrostroiExtensions учитывает за вас - сохраните себе нервы!
...
- [API](#api)
  ...
  - [Методы](#методы)
    - [Работа с ClientProps](#работа-с-clientprops)
      ...
      - [Замена модели через modelcallback - MetrostroiExtensions.UpdateModelCallback](#замена-модели-через-modelcallback---metrostroiextensionsupdatemodelcallback)
      - [Изменение созданного клиентпропа - MetrostroiExtensions.UpdateCallback](#изменение-созданного-клиентпропа---metrostroiextensionsupdatecallback)
      - [Создание нового клиентпропа - MetrostroiExtensions.NewClientProp](#создание-нового-клиентпропа---metrostroiextensionsnewclientprop)
    - [Инжект в существующие функции](#инжект-в-существующие-функции)
      - [Инжект в клиентскую функцию - MetrostroiExtensions.InjectIntoClientFunction](#инжект-в-клиентскую-функцию---metrostroiextensionsinjectintoclientfunction)
      - [Инжект в серверную функцию - MetrostroiExtensions.InjectIntoServerFunction](#инжект-в-серверную-функцию---metrostroiextensionsinjectintoserverfunction)
    - [Работа со спавнером](#работа-со-спавнером)
      ...
      - [Добавить новое поле в спавнер - MetrostroiExtensions.AddSpawnerField](#добавить-новое-поле-в-спавнер---metrostroiextensionsaddspawnerfield)
      - [Пометить клиентпроп для автоперезагрузки - MetrostroiExtensions.MarkClientPropForReload](#пометить-клиентпроп-для-автоперезагрузки---metrostroiextensionsmarkclientpropforreload)
    - [Работа с buttonmap](#работа-с-buttonmap)
      - [Описание таблицы ButtonMap](#описание-таблицы-buttonmap)
      - [Переместить существующую баттнмапу - MetrostroiExtensions.MoveButtonMap](#переместить-существующую-баттнмапу---metrostroiextensionsmovebuttonmap)
      - [Создать новую баттнмапу - MetrostroiExtensions.NewButtonMap](#создать-новую-баттнмапу---metrostroiextensionsnewbuttonmap)
...
* **Рецепт** - основная единица, с которой вы будете работать в MetrostroiExtensions. Определяет то, как изменить состав
...
### Начало
Для создания рецепта нам необходимо создать новый аддон. Рекомендуем поднять свой выделенный сервер (srcds), с установленным Metrostroi, MetrostroiExtensions и Turbostroi.
### Создание аддона
...
По образу и подобию систем в Metrostroi, первой строчкой в файле рецепта необходимо определить рецепт: `MetrostroiExtensions.DefineRecipe("<имя_рецепта")`
...
#### Замена модели через modelcallback - MetrostroiExtensions.UpdateModelCallback
`MetrostroiExtensions.UpdateModelCallback(ent, clientprop_name, new_modelcallback)` - Обновляет `modelcallback` существующего клиентпропа. 
...
**Scope**: `Client`
#### Изменение созданного клиентпропа - MetrostroiExtensions.UpdateCallback
`MetrostroiExtensions.UpdateCallback(ent, clientprop_name, new_callback)` - Обновляет `callback` существующего clientprop.
...
**Scope**: `Client`
#### Создание нового клиентпропа - MetrostroiExtensions.NewClientProp
`MetrostroiExtensions.NewClientProp(ent, clientprop_name, clientprop_info, field_name)`
...
MetrostroiExtensions позволяет удобно и без конфликтов инжектится в стандартную функцию сразу нескольким рецептам. Для этого используется система приоритетов:
...
#### Инжект в клиентскую функцию - MetrostroiExtensions.InjectIntoClientFunction
`MetrostroiExtensions.InjectIntoClientFunction(ent_or_entclass, function_name, function_to_inject, priority)` - добавляет функцию в очередь для инжекта на клиенте
...
#### Инжект в серверную функцию - MetrostroiExtensions.InjectIntoServerFunction
`MetrostroiExtensions.InjectIntoServerFunction(ent_or_entclass, function_name, function_to_inject, priority)` - добавляет функцию в очередь для инжекта на сервере
...
### Работа со спавнером
MetrostroiExtensions позволяет добавлять новые поля и инжектится в существующие без вреда другим инжектам. MetrostroiExtensions умеет автоматически пересоздавать модели при изменении значения поля спавнера. Также, при добавлении новых полей, MetrostroiExtensions умеет автоматически добавлять необходимый код для работы рандома в списках - *почти* прозрачно для программиста
#### Авторандом
Если при добавлении нового списка через [MetrostroiExtensions.AddSpawnerField](#добавить-новое-поле-в-спавнер---metrostroiextensionsaddspawnerfield) передать в качестве последнего аргумента `true`, то тогда MetrostroiExtensions автоматически пометит это поле как содержащее рандом в качестве первого элемента.
...
MetrostroiExtensions умеет автоматически пересоздавать пересоздавать клиентпропы при обновлении соответсвующего поля в спавнере. Для этого клиентпроп необходимо пометить как "требующий пересоздания" функцией [MetrostroiExtension.MarkClientPropForReload](#пометить-клиентпроп-для-автоперезагрузки---metrostroiextensionsmarkclientpropforreload)

Также, при создании клиентпропа функцией [MetrostroiExtension.NewClientProp](#создание-нового-клиентпропа---metrostroiextensionsnewclientprop) его можно сразу пометить как "требующий пересоздания" с помощью последнего аргумента - просто передайте туда имя поля
...
#### Добавить новое поле в спавнер - MetrostroiExtensions.AddSpawnerField
`MetrostroiExtensions.AddSpawnerField(ent_or_entclass, field_data, is_list_random)` - добавляет новое поле в спавнер. Автоматически передобавляет его при перезагрузке.
...
#### Пометить клиентпроп для автоперезагрузки - MetrostroiExtensions.MarkClientPropForReload
`MetrostroiExtensions.MarkClientPropForReload(ent_or_entclass, clientprop_name, field_name)` - помечает клиентпроп как "требующий перезагрузки" при изменении поля с именем `field_name`
...
#### Переместить существующую баттнмапу - MetrostroiExtensions.MoveButtonMap
`MetrostroiExtensions.MoveButtonMap(ent, buttonmap_name, new_pos, new_ang)` - перемещает существующую баттнмапу на новое место
...
#### Создать новую баттнмапу - MetrostroiExtensions.NewButtonMap
`MetrostroiExtensions.NewButtonMap(ent, buttonmap_name, buttonmap_data)` - создает новую баттнмапу
